...
LIBRSVG_CFLAGS=`pkg-config --cflags librsvg-2.0`
...
Process()
{
	...
	FAKEROOT=" -DFAKEROOT=$FAKEROOT_DIR "
        echo "Building $BUILDDIR from $SOURCEDIR with DEBUG=<$DEBUG>"
        rm -Rf ./$BUILDDIR
        mkdir $BUILDDIR || fail mkdir
        cd $BUILDDIR 
        cmake $PKG $FAKEROOT -DCMAKE_EDIT_COMMAND=rvi -DAVIDEMUX_SOURCE_DIR=$TOP -DCMAKE_INSTALL_PREFIX=/usr/local -DAVIDEMUX_INSTALL_DIR=/usr/local -DPLUGIN_UI=ALL $DEBUG -G "$BUILDER" $SOURCEDIR || fail cmakeZ
