...
jis2utf = new Iconv('ISO-2022-JP', 'UTF-8//TRANSLIT//IGNORE')
utf2jis = new Iconv('UTF-8', 'ISO-2022-JP//TRANSLIT//IGNORE')

# Preparing log stream.
fs = require('fs');
Log = require('log');
logger = new Log('notice',
    fs.createWriteStream('/var/log/hubot/hubot.log',
    {flags: 'a'}));
...
class IrcBot extends Adapter
  send: (envelope, strings...) ->
    ...
    for str in strings
      jisstr = utf2jis.convert(str).toString()
      logger.debug "#{target} #{str} -> #{jisstr}"
      @bot.say target, jisstr
  ...
  run: ->
    ...
    bot.addListener 'error', (message) ->
      logger.error('%s: %s', message.command, message.args.join(' '))
  ...
  _getTargetFromEnvelope: (envelope) ->
    ...
    logger.notice 'User: %s Room: %s', user, room
